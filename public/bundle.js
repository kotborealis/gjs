...
/***/ function(module, exports, __webpack_require__) {
	...
	var Gjs = __webpack_require__(299);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Graph = __webpack_require__(300);
	var CanvasManager = __webpack_require__(301);
	var GraphRender = __webpack_require__(302);
	...
	module.exports.Gjs = function (canvas_selector) {
	    ...
	    var canvas = new CanvasManager(canvas_selector, { fullscreen: true, enableDrag: true });
	    ...
	    var clearHEntities = function clearHEntities() {
	    ...
	    canvas.ondblclick = function (e) {
	        var node = getNodeByCoords(e.x, e.y);
	        if (!node) {
	            _this.graph.addNode(render.toViewport(e.x, e.y));
	    ...
	    var gjs_interface = {
	        clear: function clear() {
	            ...
	            render.setGraph(_this.graph);
	            clearHEntities();
	        },
	        export: function _export() {
	            window.prompt("Graph JSON:", JSON.stringify(_this.graph.export()));
	        },
	        import: function _import() {
	            gjs_interface.clear();
	            var imp = window.prompt("Graph JSON:", JSON.stringify({ nodes: [], edges: [] }));
	            try {
	                _this.graph.import(JSON.parse(imp));
	            } catch (e) {
	                alert("Invalid Graph JSON! " + e.toString());
	            }
	        },
	        export_to_file: function export_to_file() {
	            ...
	            var blob = new Blob([JSON.stringify(_this.graph.export())], { type: "application/json;charset=utf-8" });
	            saveAs(blob, 'graph_' + Date.now() + '_' + _this.graph.nodes.size + 'n_' + _this.graph.edges.size + 'e.gjs.json');
	        },
	        import_from_file: function import_from_file() {
	            ...
	            input.addEventListener('change', function (e) {
	                ...
	                reader.onload = function (e) {
	                    try {
	                        gjs_interface.clear();
	                        _this.graph.import(JSON.parse(e.target.result));
	                    } catch (e) {
	                        alert("Invalid Graph JSON! " + e.toString());
	                    }
	                };
...
/***/ function(module, exports) {
	...
	module.exports = function () {
	    ...
	    var NODE_ID_GEN_SEQ = 0;
	    var EDGE_ID_GEN_SEQ = 0;
	    ...
	    var addNodeHelper = function addNodeHelper(node) {
	        if (node.id === undefined) node.id = NODE_ID_GEN_SEQ++;
	        if (_this.nodesIndex.has(node.id.toString())) throw new Error("Node with id " + node.id.toString() + " already exists");
	
	        NODE_ID_GEN_SEQ = Number.isNaN(Number.parseInt(node.id)) ? NODE_ID_GEN_SEQ : Math.max(Number.parseInt(node.id) + 1, NODE_ID_GEN_SEQ);
	    ...
	    var addEdgeHelper = function addEdgeHelper(edge) {
	        if (edge.id === undefined) edge.id = EDGE_ID_GEN_SEQ++;
	        ...
	        if (!_this.nodesIndex.has(edge.s.toString()) || !_this.nodesIndex.has(edge.t.toString())) throw new Error("Edge target/source (" + edge.s.toString() + "/" + edge.t.toString() + ") node does not exists");
	
	        EDGE_ID_GEN_SEQ = Number.isNaN(Number.parseInt(edge.id)) ? EDGE_ID_GEN_SEQ : Math.max(Number.parseInt(edge.id) + 1, EDGE_ID_GEN_SEQ);
	    ...
	    this.getNode = function (id) {
	        return _this.nodesIndex.get(id.toString());
	    };
	    this.getEdge = function (id) {
	        return _this.edgesIndex.get(id.toString());
	    };
	    ...
	    this.import = function (data) {
	        _this.addNode(data.nodes);
	        _this.addEdge(data.edges);
	    };
	};
...
/***/ function(module, exports) {
	...
	module.exports = function (canvas) {
	    ...
	    window.onresize = function () {
	        ...
	        _this.height = _this.e_canvas.height;
	        _this.clear();
	        _this.onresize();
	    };
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports.Render = function (canvasManager, graph) {
	    ...
	    if (!(canvasManager instanceof CanvasManager)) throw new Error("First argument of GraphRender.Render must be CanvasManager instance");
	    if (!(graph instanceof Graph)) throw new Error("Second argument of GraphRender.Render must be Graph.Graph instance");
	    ...
	    this.setGraph = function (new_graph) {
	        if (!(new_graph instanceof Graph)) throw new Error("Second argument of GraphRender.Render must be Graph.Graph instance");
	        graph = new_graph;
	    };
